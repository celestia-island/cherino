...
const RUN_DIR: &str = "/run/cherino/youki";
const FALLBACK_RUN_DIR: &str = "/tmp/cherino/youki";
/// Generic env override for the youki run dir; brand and legacy names are
/// kept as compat fallbacks, in precedence order: `CONTAINER_RUN_DIR`, then
/// `CHERINO_RUN_DIR`, then the deprecated entelecheia-era
/// `ENTELECHEIA_RUN_DIR` (warns when used).
const RUN_DIR_ENV: &str = "CONTAINER_RUN_DIR";
const CHERINO_RUN_DIR_ENV: &str = "CHERINO_RUN_DIR";
...
fn resolve_run_dir() -> PathBuf {
    pick_run_dir(
        ...
        std::env::var(CHERINO_RUN_DIR_ENV).ok().as_deref(),
...
/// explicit generic env always wins, then the cherino brand env; otherwise
/// probe the primary dir and fall back to the legacy entelecheia-era env
/// override (deprecated, warns), then the fallback path.
fn pick_run_dir(
    generic_env: Option<&str>,
    cherino_env: Option<&str>,
    ...
    if let Some(run_dir) = cherino_env.filter(|value| !value.is_empty()) {
        warn!(run_dir = %run_dir, "using cherino run dir env override");
        return PathBuf::from(run_dir);
    }
    ...
    if let Some(run_dir) = legacy_env.filter(|value| !value.is_empty()) {
        warn!(
            ...
            fallback = %run_dir,
            env = LEGACY_RUN_DIR_ENV,
            new_env = CHERINO_RUN_DIR_ENV,
            "run_dir not writable, using deprecated entelecheia-era env override \
             (rename it to {})",
            CHERINO_RUN_DIR_ENV
        );
        return PathBuf::from(run_dir);
...
impl YoukiManager {
    ...
    async fn walk_diff(
        ...
        changes: &mut Vec<cherino::PathChange>,
    ) -> ContainerResult<()> {
                ...
                })?
        {
            ...
            match (host_meta, container_meta) {
                (None, Ok(cm)) => {
                    changes.push(cherino::PathChange {
                        path: relative,
                        kind: cherino::ChangeKind::Added,
                    });
                    if cm.is_dir() {
                ...
                (Some(hm), Ok(cm)) => {
                    if hm.len() != cm.len() {
                        changes.push(cherino::PathChange {
                            path: relative,
                            kind: cherino::ChangeKind::Modified,
                        });
                ...
                (Some(_), Err(_)) => {
                    changes.push(cherino::PathChange {
                        path: relative,
                        kind: cherino::ChangeKind::Deleted,
                    });
                }
                _ => {}
            }
...
impl ContainerOps for YoukiManager {
    ...
    async fn writable_rootfs(
        &self,
        container_id: &str,
    ) -> ContainerResult<cherino::WritableRootfs> {
        let resolved_id = self.resolve_id(container_id).await?;
        let record = self
            .state
            .get(&resolved_id)
        ...
        if merged.is_dir() {
            return Ok(cherino::WritableRootfs {
                path: merged,
                is_direct: true,
            });
        }
    ...
    async fn diff_workspace(
        ...
        base_path: &str,
    ) -> ContainerResult<Vec<cherino::PathChange>> {
...
mod tests {
    ...
    async fn test_diff_workspace_with_nested_added_dir() -> Result<()> {
        ...
        let added: Vec<String> = changes
            .iter()
            .filter(|c| c.kind == cherino::ChangeKind::Added)
            .map(|c| c.path.to_string_lossy().to_string())
            .collect();
...
mod env_config_tests {
    ...
    #[test]
    fn run_dir_cherino_env_beats_legacy_and_probe() {
        let dir = pick_run_dir(
            None,
            Some("/tmp/cherino"),
            Some("/tmp/legacy"),
        ...
        assert_eq!(dir, PathBuf::from("/tmp/cherino"));
        let dir = pick_run_dir(
            Some(""),
            Some("/tmp/cherino"),
            None,
    ...
    fn run_dir_uses_writable_primary() {
        let primary =
            std::env::temp_dir().join(format!("cherino-run-dir-test-{}", std::process::id()));
