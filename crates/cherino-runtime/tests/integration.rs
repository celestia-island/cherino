...
use cherino::{ops::ContainerOps, types::ContainerCreateParams};

fn data_dir() -> PathBuf {
    // Standalone replacement for the plana-era `_config::UserConfig::config_dir()`
    // dev dependency: integration tests are root-only and ephemeral, so a
    // per-process temp dir is sufficient.
    std::env::temp_dir().join(format!("cherino-runtime-it-{}", std::process::id()))
}

#[tokio::test]
#[ignore = "requires root: sudo cargo test -p cherino_runtime --test integration -- --ignored"]
async fn test_full_lifecycle() -> Result<()> {
    let mgr = cherino_runtime::YoukiManager::new(&data_dir()).context("create YoukiManager")?;
...
async fn test_host_rootfs_overlay() -> Result<()> {
    ...
    let rootfs_mgr = cherino_runtime::RootfsManager::new(&data_dir());
